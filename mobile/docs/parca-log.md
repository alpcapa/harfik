# Web ↔ Uygulama Arasındaki Kabul Edilmiş Farklar — Parça Günlüğü

> mobile/docs/'e taşındı (context split, 24 Ağustos 2026). Bu, portun Parça 1'den bugüne kadarki TÜM UI/davranış paritesi kararlarının kronolojik kaydıdır (Parça 1...131+) — en büyük tek kaynak (mobile/CLAUDE.md'nin ~yarısı). Kök mobile/CLAUDE.md'deki kod yorumlarındaki 'bkz. mobile/CLAUDE.md, Parça N' atıfları artık BURAYA işaret eder.
...
## Sıradaki Fazlar (mutabık kalınan sıra) — Parça günlüğünün devamı

> Yukarıdaki 'Web ↔ Uygulama Arasındaki Kabul Edilmiş Farklar' bölümüyle AYNI
> Parça numaralandırmasını sürdürüyor (Parça N atıfları burada da geçerli) —
> orijinal dosyada farklı bir başlık altındaydı, context split sırasında aynı
> günlüğe eklendi ki 'Parça N nerede?' sorusunun TEK cevabı olsun.

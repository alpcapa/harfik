...
// (Zor Faz 5'e kadar YOK), Kolay açıklaması ve HelpModal'ın zorluk paragrafı
...
void main() {
  ...
  test('Kolay açıklaması Setup.tsx ile birebir', () {
    final setup = File('../../src/components/Setup.tsx').readAsStringSync();
    // JSX: `<p …>` içindeki metin, satır sonlarındaki boşluklarla.
    final m = RegExp(r"Kolay'da Yapay Zeka[^<]*").firstMatch(setup);
    expect(m, isNotNull, reason: 'Setup.tsx Kolay açıklaması bulunamadı');
    expect(_norm(kolayAciklamasi), _norm(m!.group(0)!));
  });

-- Bir önceki migration'daki (`admin_game_activity_include_online`)
-- `admin_game_activity_series` yeniden yazımı canlıda hemen hata verdi:
-- `sum(integer)/count(bigint)` bigint/bigint tamsayı bölmesine düşüyordu
-- (avg_duration_same_session_seconds için) ve fonksiyonun bildirdiği `numeric`
-- dönüş tipiyle eşleşmiyordu ("structure of query does not match function
-- result type... Returned type bigint does not match expected type numeric").
-- Üç ortalama-süre ifadesindeki bölme işlemlerinin payına `::numeric` cast'i
-- eklendi.

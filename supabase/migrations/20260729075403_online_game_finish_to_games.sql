-- Kelimeki — Canlı oyun bitişini `games` tablosuna bağla.
--
-- Şimdiye kadar online_games.status='finished' olduğunda hiçbir yerel
-- `games` kaydı oluşmuyordu — Sanal Lig puanı/Skor Kartı/Oyun Geçmişi'ne
-- hiç yansımıyordu. Yerel oyundaki buildGameRecord (src/utils/gameRecord.ts)
-- yalnızca hesap sahibini (0. koltuk) kaydeder çünkü hotseat'te diğer
-- "insan" oyuncular aynı cihazı paylaşır, kendi hesapları yoktur. Canlı
-- oyunda durum farklı: her insan koltuk GERÇEKTEN farklı bir hesap
-- olabilir, bu yüzden submit_move'un bitiş dalı artık HER insan koltuk
-- için kendi hesabına ayrı bir games satırı açıyor.
--
-- Rank/sonuç mantığı rankPlayers'daki (src/utils/ranking.ts) "competition
-- ranking" ile birebir aynı: skora göre azalan sıra, eşit puan aynı sırayı
-- paylaşır. Surrender online'da henüz desteklenmediğinden (bkz. CLAUDE.md
-- "Bilinçli kapsam dışı bırakılan") tüm satırlar surrendered=false ile
-- yazılıyor — ileride surrender eklenince bu fonksiyon da güncellenmeli.
--
-- board_snapshot bilinçli olarak NULL bırakıldı (v1) — client tarafındaki
-- serializeBoardSnapshot'ın SQL karşılığını yazmak ayrı bir adım; mevcut
-- GameHistoryModal zaten null-toleranslı (eski kayıtlarda da hep null).

alter table public.games
  add column if not exists online_game_id uuid references public.online_games (id) on delete set null;

comment on column public.games.online_game_id is 'Doluysa bu kayıt bir Canlı (çok hesaplı) oyundan geldi — GameHistoryModal bu oyunları görsel olarak ayırt etmek için kullanır.';

create index if not exists games_online_game_id_idx on public.games (online_game_id) where online_game_id is not null;
  ...
  if v_is_game_over then
    ...
    for i in 0 .. v_player_count - 1 loop
      v_fp_players_json := v_fp_players_json || jsonb_build_array(jsonb_build_object(
        'name', (v_new_players -> i ->> 'name'),
        'score', (v_new_players -> i ->> 'score')::int,
        'is_ai', (v_new_players -> i ->> 'isAI')::boolean,
        'surrendered', (v_new_players -> i ->> 'surrendered')::boolean,
        'colorIndex', (v_new_players -> i ->> 'colorIndex')::int
      ));
    end loop;
    ...
    for i in 0 .. v_player_count - 1 loop
      ...
      if (v_fp_slot ->> 'type') = 'human' then
        ...
        insert into public.games (
          ...
          players, longest_word, online_game_id
        ) values (

-- Canlı oyun hamlelerine istemci üretimli idempotency anahtarı (Flutter portu
-- hazırlığı, bkz. mobile/CLAUDE.md "Üst Düzey Kararlar" #2a).
--
-- Sorun: mobil ağlarda "istek gitti, yanıt kayboldu" olağandır. submit_move
-- başarıyla işlendikten sonra istemci aynı hamleyi yeniden denerse bugüne
-- kadar 'Sıra sende değil.' hatası alıyordu — gerçek bir retle (sıra gerçekten
-- başkasında) ayırt edilemez. Çözüm, saveGame'in 23505 deseniyle aynı fikir:
-- istemci her hamleye bir UUID (p_move_id) verir; aynı UUID'yle gelen yeniden
-- deneme, hamle zaten kayıtlıysa HİÇBİR ŞEY yapmadan başarı döner.
--
-- Geriye dönük uyumluluk: p_move_id DEFAULT NULL — mevcut web istemcisi ve
-- play-ai-turn Edge Function'ı parametreyi hiç göndermeden aynen çalışmaya
-- devam eder (null → dedup atlanır, davranış bugünküyle birebir aynı).
-- Parametre EKLEMEK overload yaratacağından (PostgREST'te 8-argümanlı çağrı
-- iki fonksiyona da uyar → 300 ambiguous) eski imza DROP edilip tek fonksiyon
-- olarak yeniden CREATE edilir — list_my_online_games'teki aynı ders.
--
-- Dedup kontrolü online_games satır kilidinden SONRA yapılır: eşzamanlı iki
-- özdeş deneme kilitte serileşir, ikincisi ilkinin commit'lediği hamle
-- satırını görür. player_user_id = auth.uid() şartı, başka bir katılımcının
-- (kötü niyetle) benim move_id'mi kullanarak kendi hamlesini sessizce yutma
-- ihtimalini kapatır. Kısmi unique index kemer+pantolon askısı: dedup bir
-- şekilde atlanırsa çifte uygulama sessizce değil, gürültüyle (23505) ölür.

alter table public.online_game_moves add column move_id uuid;

create unique index online_game_moves_game_move_id_key
  on public.online_game_moves (online_game_id, move_id)
  where move_id is not null;

drop function public.submit_move(uuid, text, jsonb, jsonb, jsonb, jsonb, integer, jsonb);

create function public.submit_move(
  p_game_id uuid,
  p_action text,
...
-- Grant'ler drop ile kaybolur — mevcut ACL birebir geri kurulur
-- (bkz. 20260725053640_revoke_toggle_game_favorite_anon dersi).
revoke all on function public.submit_move(uuid, text, jsonb, jsonb, jsonb, jsonb, integer, jsonb, uuid) from public, anon, authenticated;
grant execute on function public.submit_move(uuid, text, jsonb, jsonb, jsonb, jsonb, integer, jsonb, uuid) to authenticated, service_role;

-- Kelimeki — uygulama içinden hesap silme: veritabanı kaskadı.
-- (ROADMAP madde 2 — MAĞAZA BLOKERİ; Apple 5.1.1(v) + Google'ın veri silme şartı.)
--
-- BU FONKSİYON KULLANICIYA AÇIK DEĞİL. `delete-my-account` Edge Function'ı
-- çağıranın KENDİ JWT'siyle kimliğini doğrular, sonra service-role ile burayı
-- çağırır. `authenticated`/`anon` rollerinden execute yetkisi bilerek geri
-- alınıyor: bir uuid parametresi alan ve o kullanıcının her şeyini silen bir
-- fonksiyon, RLS'in koruyamayacağı bir yüzeydir.
--
-- ── VERİLMİŞ KARAR (25 Ağustos 2026, kullanıcı: "Anonimleştirme mantıklı") ──
--   Silinen kişinin KENDİ `games` satırları SİLİNİR (istatistikleri de böyle
--   gider, amaç bu). BAŞKALARININ satırları KORUNUR, jsonb'deki adı
--   ANONİMLEŞTİRİLİR (hem `players` hem `messages`).
--   Gerekçe ölçüldü: `games` oyun başına değil OYUNCU BAŞINA satır tutuyor ve
--   `player_stats_overall` bir VIEW olarak `GROUP BY user_id` yapıyor — yani
--   silmek kimsenin puanını düşürmüyor.
--
-- ── ADI EŞLEŞTİREREK BULMA (ölçülmüş tuzak) ────────────────────────────────
--   Snapshot'ın alanları `name`, `score`, `colorIndex`, `is_ai`,
--   `surrendered` — `user_id` YOK (`messages` de aynı). Ada göre eşleştirmek
--   ad çakışmasında ve sonradan yapılan ad değişikliklerinde YANLIŞ kişiyi
--   anonimleştirir. Doğru anahtar KOLTUK İNDEKSİ: `online_games.slots[i]`
--   ↔ `colorIndex = i`. Bu değişmez projede zaten yazılı ve bir kez daha
--   kullanılıyor (bkz. `chat_flags_for_finished_game`,
--   `person_scoped_chat_moderation` migration'ı: "colorIndex,
--   init_online_game_state'te i % 4 ile atanır ve oyuncu sayısı en fazla 4
--   olduğundan koltuk indeksiyle BİREBİR aynıdır").
--
-- ── "ZATEN CASCADE EDER" VARSAYIMI İKİ YERDE YANLIŞ ÇIKTI ──────────────────
--   1. `online_games.created_by` → `on delete CASCADE`. Yani `auth.users`
--      satırını silmek, o kişinin AÇTIĞI BİTMİŞ oyunları da silerdi; bu da
--      `games.online_game_id`'yi (SET NULL) koparıp ÖTEKİ oyuncunun sohbet
--      arşivi rozetlerini (`chat_flags_for_finished_game`) ve o oyunun
--      hamle/mesaj kayıtlarını yok ederdi. Aşağıda SET NULL'a çevriliyor.
--      RLS karşılaştırmaları (`created_by = auth.uid()`) null'da zaten
--      eşleşmiyor, yani gevşeme YOK.
--   2. Dört tablonun FK'si NO ACTION (ne cascade ne set null):
--      `online_game_messages.sender_user_id` (NOT NULL → satır silinmeli),
--      `online_game_moves.player_user_id` (NULLABLE → SET NULL, hamle
--      geçmişi ötekinin verisi, korunur), `online_game_message_mutes` ve
--      `online_game_chat_reports` (ikisinde de iki kolon da NOT NULL →
--      satır silinmeli). Bunlar temizlenmezse `auth.admin.deleteUser`
--      doğrudan FK ihlaliyle DÜŞER.
--
-- ── DEVAM EDEN OYUNLAR ─────────────────────────────────────────────────────
--   Koltuğu boşalan bir oyun oynanamaz. Kural: kişinin katıldığı bir
--   `online_games` satırı, ONA AİT HİÇBİR `games` satırı yoksa (yani oyun
--   hiç bitmemiş, kimsenin dondurulmuş kaydı yok) SİLİNİR; bitmiş oyunlar
--   KORUNUR. Böylece "başkasının bitmiş oyun kaydı silinmez" kuralı
--   duruyor, yarım oyun da ortada asılı kalmıyor.
--
-- ── DOKUNULMAYANLAR (bilinçli) ─────────────────────────────────────────────
--   `game_finishes.user_id` → SET NULL (huninin süre/oyuncu sayısı ölçümü
--   kimliksiz kalıyor, tasarım gereği). `guest_visits`/`device_visits`/
--   `game_starts`/`client_errors` hesapla HİÇ ilişkilendirilmiyor
--   (`anon_id`), Gizlilik Politikası bunu zaten böyle anlatıyor.
--   `online_games.slots` içindeki uuid bırakılıyor: `auth.users` ve
--   `profiles` gittikten sonra o değer hiçbir kişiye çözülemez, ama koltuk
--   eşlemesi ötekinin arşivi için gerekli.

-- ── 1) online_games.created_by: CASCADE → SET NULL ─────────────────────────
alter table public.online_games alter column created_by drop not null;
alter table public.online_games drop constraint online_games_created_by_fkey;
alter table public.online_games
  add constraint online_games_created_by_fkey
  foreign key (created_by) references auth.users (id) on delete set null;

comment on column public.online_games.created_by is
  'Oyunu açan kullanıcı. Hesap silinirse NULL olur (SET NULL) — CASCADE olsaydı '
  'o kişinin açtığı BİTMİŞ oyunlar da silinir, öteki oyuncunun games satırının '
  'online_game_id bağı kopardı. RLS "created_by = auth.uid()" null''da eşleşmez.';

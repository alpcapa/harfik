-- Kelimeki — oyunu BİTİREN hamle games.moves arşivine hiç girmiyordu
--
-- Kullanıcı 15 Ağustos 2026'da bildirdi: bitmiş bir Canlı oyunun "Hamleler"
-- dökümünde en son kendi oynadığı kelime görünüyor, oysa oyunu rakibin son
-- hamlesi bitirmiş. Oyun DOĞRU bitmişti (end_reason 'normal', rakip rafını
-- torba boşken bitirdi) — kusur yalnızca ARŞİVDE.
--
-- Kök sebep SIRALAMA: `_finish_online_game_records` arşivi
-- `_online_moves_snapshot(p_game_id)` ile `online_game_moves` tablosundan
-- üretiyor, ama HEM `submit_move` HEM `check_turn_timeout` bu fonksiyonu o
-- turun kendi satırını insert etmeden ÖNCE çağırıyordu. Sonuç: arşiv her
-- zaman son hamle eksik donuyor.
--
-- Kapsam ölçüldü: arşivi olan 36 Canlı oyunun 6'sı eksik — hepsi
-- `end_reason 'normal'` ve hepsi 12 Ağustos 2026'dan (games.moves kolonunun
-- eklendiği tarihten) SONRA bitmiş. Daha eski 30 oyun `games_moves_snapshot`
-- migration'ının geriye dönük doldurmasıyla geldiğinden (o, hamleler zaten
-- yazıldıktan sonra çalıştı) eksiksiz. `check_turn_timeout`'taki aynı hata
-- bugüne kadar VERİDE GÖRÜNMEDİ (o tarihten beri hiç teslimle biten oyun
-- arşivlenmedi) — yani gizli/uykuda bir hataydı, önleyici olarak düzeltiliyor.
--
-- Etkilenmeyenler: `board_snapshot` (bellekteki v_board'dan), skor/rank
-- (bellekteki p_players'tan) ve `messages` — hiçbiri online_game_moves'a
-- bakmıyor.

create or replace function public.submit_move(
  p_game_id uuid,
  p_action text,
  p_placements jsonb default null::jsonb,
  p_exchange_letters jsonb default null::jsonb,
  p_words jsonb default '[]'::jsonb,
  p_word_scores jsonb default null::jsonb,
...
begin
  ...
  -- İdempotent yeniden deneme: bu move_id ile (bu kullanıcının) hamlesi zaten
  -- işlenmişse hiçbir şey yapmadan başarı dön — yanıtı kaybolan istemcinin
  -- retry'ı çifte hamle de üretmez, sahte 'Sıra sende değil.' reddi de almaz.
  if p_move_id is not null and exists (
    select 1 from public.online_game_moves m
    where m.online_game_id = p_game_id
      and m.move_id = p_move_id
      and m.player_user_id = v_uid
  ) then
    return;
  end if;
  ...
  insert into public.online_game_moves (
    ...
    finish_joker_count, bingo, move_id
  ) values (
  ...
  if v_is_game_over then
    -- ÖNEMLİ: arşiv (games.moves) online_game_moves'tan üretildiğinden bu çağrı
    -- YUKARIDAKİ insert'ten SONRA gelmek ZORUNDA. Önceden rack cezası hesabının
    -- hemen ardında duruyordu; oyunu BİTİREN hamle arşive hiç girmiyordu.
...
-- Aynı sıralama hatası burada da vardı: teslim satırı fonksiyonun EN SONUNDA
-- insert ediliyordu, oysa `_finish_online_game_records` ondan önce çağrılıyor.
-- Insert artık if/else'ten ÖNCE — iki dal da (oyun bitti / devam ediyor) aynı
-- satırı yazdığından davranış değişmiyor, yalnızca sırası öne alınıyor.
...
begin
  ...
  -- _finish_online_game_records arşivi online_game_moves'tan üretiyor. Aynı sıralama
  -- hatası submit_move'da da vardı (bkz. bu migration'ın submit_move kısmı).
  insert into public.online_game_moves (
    online_game_id, turn, player_index, player_user_id, action,
    words, word_scores, points, lost_shares, tile_count, placements,
...
-- Geriye dönük düzeltme: eksik arşivi olan satırları online_game_moves'tan
-- yeniden üret. Canlı hamle tablosu silinmediğinden kayıp veri yok.
update public.games g
set moves = public._online_moves_snapshot(g.online_game_id)
where g.online_game_id is not null
  and g.moves is not null
  and (select max((e ->> 'turn')::int) from jsonb_array_elements(g.moves) e)
      is distinct from (select max(m.turn) from public.online_game_moves m
                        where m.online_game_id = g.online_game_id);

-- Kelimeki — submit_move'un oyun-bitişi `games.players` snapshot sırası düzeltmesi.
--
-- Bulunan hata (29 Temmuz 2026, gerçek 4 kişilik oyunda test edilirken):
-- kullanıcı Skor Kartı > Tüm Oyunlarım listesinde yanlış bir sıralama
-- gördü — berabere kalan 2. ve 3. oyuncular 3. sırada gösteriliyor,
-- gerçek sonuncu oyuncu 2. sırada görünüyordu. Kök sebep: bir önceki
-- migration'daki (20260729075403) v_fp_players_json inşası oyuncuları
-- KOLTUK sırasıyla (seat 0,1,2,...) diziyordu. Client tarafındaki
-- computeRanks (src/utils/leaguePoints.ts) SIRALAMA YAPMIYOR — yalnızca
-- ardışık elemanları karşılaştırıp aynı skor/surrendered ise aynı sırayı
-- veriyor, yani girdinin ZATEN skora göre azalan sırada (rank sırasında)
-- olduğunu varsayıyor. Yerel oyunlar bu varsayımı buildGameRecord'daki
-- `players: ranked.map(...)` ile (src/utils/gameRecord.ts) sağlıyor;
-- online oyunlar sağlamıyordu. Düzeltme: players snapshot'ı da rankPlayers
-- (src/utils/ranking.ts) ile aynı mantıkla — skora göre azalan, eşitlikte
-- orijinal koltuk sırası korunarak (stable sort) — inşa ediliyor.
--
-- Rank/result skaler sütunları (games.rank/result) zaten doğruydu, yalnızca
-- players jsonb dizisinin eleman SIRASI yanlıştı — bu migration yalnızca
-- o inşa bloğunu değiştiriyor, fonksiyonun geri kalanı aynı.

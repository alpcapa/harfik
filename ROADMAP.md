...
| **1.0.6'ya binen kod** | Biten Canlı oyunun haberi (`OYUN BİTTİ`/`TESLİM OLDUN` + `YENİ` rozeti + sekme sayacı) · skor kartında kafa kafaya oran çubuğu · `Tüm Oyunlar` etiketinin tekleşmesi · **oyun geçmişine "Tekrar Oyna" (rövanş)** | ⏳ **`1.0.6 (525) — 711eaaa` kapalı testte YAYINDA** (4 Eylül, Submission 12; inceleme ≤29 dk). Kullanıcı kuralı sağlandı: APK önce cihazda koşuldu (§0-§4'ün koşulabilir maddeleri geçti). **TUR HENÜZ KAPANMADI** — üç madde yalnızca PLAY'den kurulmuş pakette denenebilir: §4.1 (App Links ile kayıt onayı) · §1.4 ("ŞİMDİ DEĞİL" dalı) · davet linkinin uygulamayı açması. Kanonik paket kütüğü: `mobile/docs/surumler.md` |
